...
namespace phase_picker {
	...
	struct smooth_function_t {
		static constexpr unsigned BASE_WINDOW_SIZE = 40;
	...
	private:
			...
			pphase_picker_result p_picker_result)
		{
			...
			std::ofstream d_energy_damping_f("s_d_energy_damping.bin", std::ios::binary);
			d_energy_damping_f.write(reinterpret_cast<char*>(d_energy_damping.data()), d_energy_damping.size()*sizeof(T));
			d_energy_damping_f.close();
			...
			const VectorX<decltype(nbins)> nbins_seq = VectorX<decltype(nbins)>::LinSpaced((nbins - 15) / 5, 15, nbins);
			...
			{
				sp_loc_all[i_bin] = get_lock_hist(d_energy_damping, nbins_seq[i_bin], PICKER_TYPE::S_PICKER);
		...
		template <typename T>
		auto prepare_signal(const Map<VectorX<T>> &x) -> std::pair<VectorX<T>, VectorX<T>> 
		{
			VectorX<T> signal = x;
		...
		template <typename T>
		Matrix<T, Dynamic, Dynamic> construct_sdof_system(const VectorX<T> & signal,
			double Tn,
		...
		auto find_kink(const VectorX<T> &energy_damping,
			...
			std::pair<idx_t<T>, idx_t<T>> p_s_locks,
			idx_t<T> interval_after_supposed_s_pick,
			double min_edi_ratio) -> std::pair<T, idx_t<T>>
		{
			...
			if (max_peak_after_s_candidate.first > min_edi_ratio*peak_before_s_candidate) {
			...
			PICKER_TYPE picker_t)
		{
			...
			Vector2d R = statelevel(d_energy_damping, bin_size, picker_t);
		...
		Vector2d statelevel(const VectorX<T>& y,
			...
			if (picker_t == PICKER_TYPE::P_PICKER) {
				...
				//Skip first bin 
				l_hist.tail(1).maxCoeff(&i_max_l_hist);
				++i_max_l_hist;
				u_hist.tail(1).maxCoeff(&i_max_u_hist);
				++i_max_u_hist;
			}
